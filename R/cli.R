...
get_domain_specific_options <- function() {
  breeding_status_path <- geci.optparse::character_option(c("-b", "--breeding-status"), default = "/workdir/breeding_status.csv", help = "File path of the breeding_status_database")
  tracking_path <- geci.optparse::character_option(c("-c", "--tracking-data"), default = "/workdir/gps-albatros-guadalupe.csv", help = "File path of the tracking database")
